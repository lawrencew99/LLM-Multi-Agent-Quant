...
def bull_researcher(state: TradingState) -> dict[str, Any]:
    """LangGraph node: build the strongest bull thesis from analyst reports."""
    ...
    try:
        ...
        log.exception("bull_researcher_failed", ticker=state["ticker"])
        return {
            "bull_arguments": [{"error": str(exc), "round": round_num, "stance": "bull", "conviction": 0.0}],
